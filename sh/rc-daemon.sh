...
update_service_status() {
	...
	for (( i=0; i<${#RC_DAEMONS[@]}; i++ )); do
		local opts="--test --stop"
		[[ -n ${RC_DAEMONS[i]} ]] && opts="${opts} --exec \"${RC_DAEMONS[i]}"\"
		[[ -n ${RC_PIDFILES[i]} ]] && opts="${opts} --pidfile \"${RC_PIDFILES[i]}\""
		[[ -n ${RC_NAMES[i]} ]] && opts="${opts} --name \"${RC_NAMES[i]}\""
